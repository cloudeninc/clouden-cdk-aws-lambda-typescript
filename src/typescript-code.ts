...
export class TypeScriptAssetCode extends AssetCode {
  ...
  private async typeScriptBuild() {
    ...
    const stamp = pathModule.join(this.path, ".stamp");
    if (fs.existsSync(stamp)) {
      const mtime = fs.statSync(stamp).mtime;
      const newer_files = await recursive(this.typeScriptSourcePath, [
        (file, stats) => {
          return stats.isDirectory() || stats.mtime <= mtime;
        },
      ]);
